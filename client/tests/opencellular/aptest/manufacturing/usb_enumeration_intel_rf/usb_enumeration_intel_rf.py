...
class usb_enumeration_intel_rf(test.test):
    ...
    def run_once(self, test_path='', rasp_ip=None, user=None, relay_num=None, on_off=None):
        ...
        """
        logging.info('TEST CASE XX - AP Test - Manufacturing - Check USB enumeration and data speed between Intel and RF')   
        ...
        pid_status = subprocess.check_output('/usr/local/osmo_built/lib/uhd/utils/uhd_find_devices', shell=True) 
        time.sleep(5)
        logging.info('pid status is : %s',pid_status)

        
        if 'No UHD Devices Found' in pid_status:
            logging.info('NO UHD DEVICES FOUND')
			raise ValueError('WRONG DATA') 
        else:
            logging.info('USB Enumeration and Data speed between Intel and RF :\n')
