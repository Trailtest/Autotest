...
class rw_fpga_eeprom_vidpid(test.test):
    ...
    def run_once(self, test_path='', rasp_ip=None, user=None, relay_num=None, on_off=None):
        ...
        """
        logging.info('TEST CASE XX - AP Test - Manufacturing - Write Read FPGA EEPROM for VID and PID')   
        ...
        pid_status = subprocess.check_output('/usr/local/osmo_built/lib/uhd/utils/lsusb', shell=True) 
        time.sleep(5)
        logging.info('pid status is : %s',pid_status)

        
        if 'ID 2500:0020' in pid_status:
            logging.info('FPGA READ WRITE OPERATION SUCCESS')
        else:
            logging.info('FPGA READ WRITE OPERATION FAILED')
