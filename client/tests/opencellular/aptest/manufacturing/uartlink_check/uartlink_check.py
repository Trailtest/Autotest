...
class uartlink_check(test.test):
    ...
    def run_once(self, test_path='', rasp_ip=None, user=None, relay_num=None, on_off=None):
        ...
        """
        logging.info('TEST CASE XX - AP Test - Manufacturing - UART Link Check Test')   
	...
	oc_ip = oc_config.get('oc', 'ip')
        ...
        output = subprocess.check_output('/home/oc/host/ocmw/occmd echo system', shell=True) 
        time.sleep(5)

        logging.info(output)        
        if 'Success' in output:
            logging.info('UART LINK BETWEEN INTEL & TIVA IS ACTIVE')
        else:
            logging.info('UART LINK BETWEEN INTEL & TIVA IS NOT ACTIVE')
