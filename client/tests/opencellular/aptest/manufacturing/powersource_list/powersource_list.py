...
class powersource_list(test.test):
    ...
    def run_once(self, test_path='', rasp_ip=None, user=None, relay_num=None, on_off=None):
        ...
        """
        logging.info('TEST CASE XX - AP Test - Power source list')
        ...
        time.sleep(5)
        logging.info('COLLECTING Power Source List ....')
        # executing power source list command on oc
        output = subprocess.check_output(
            '/home/oc/host/ocmw/occmd get status powersource', shell=True)
        time.sleep(5)

        # collecting sdr temperature from output
        logging.info('Avialable power source list :%s', output)
        time.sleep(5)
        # s.logout()
        s.close()
